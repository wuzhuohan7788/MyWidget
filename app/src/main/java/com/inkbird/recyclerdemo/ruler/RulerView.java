package com.inkbird.recyclerdemo.ruler;
...
public class RulerView extends View implements ScrollChange {
    ...
    private void init(Context context) {
        ...
        mTextPaint.setColor(Color.parseColor("#F37813"));
        ...
        mLinePaint.setColor(Color.parseColor("#F37813"));

        mLineRectPaint = new Paint();
        mLineRectPaint.setAntiAlias(true);
        mLineRectPaint.setStyle(Paint.Style.FILL);
        mLineRectPaint.setColor(Color.parseColor("#F37813"));
    ...
    private void initDistanceForDp() {
        mLineWidth = dp2px(2);
        mLineSpace = dp2px(10);
        mSmallLineHeight = dp2px(15);
        mLongLineHeight = dp2px(30);
        dpFor05 = dp2px(0.5f);
        dpFor14 = dp2px(14);
    }
     ...
     * @param end
     */
    public void setScope(int start, int end, int offSet, String unit) {
        this.unit = unit;
    ...
    private void drawRuler(Canvas canvas) {
        ...
        for (int index = 0; index <= mRulerHelper.getCounts(); index++) {
            ...
            mRect.top = getTopStartY(longLine);
            mRect.right = mRect.left + mLineWidth;
            mRect.bottom = getTopEndY();
            canvas.drawRect(mRect, mLinePaint);

//            //底部刻度
//            mRect.left = index * mLineSpace + lineCount + mMarginLeft;
//            mRect.top = getBottomStartY();
//            mRect.right = mRect.left + mLineWidth;
//            mRect.bottom = getBottomEndY(longLine);
//            canvas.drawRect(mRect, mLinePaint);
            mRulerHelper.addPoint(mRect.left);
            if (longLine) {
                if (!mRulerHelper.isFull()) {
                    String text = mRulerHelper.getTextByIndex(mTextIndex) + unit;
                    mTextIndex += 5;
                    Paint.FontMetrics fm = mTextPaint.getFontMetrics();
                    canvas.drawText(text, mRect.centerX(), getTopEndY() + dp2px(10) - (fm.descent - (-fm.ascent + fm.descent) / 2), mTextPaint);
                }
            }
            mRect.setEmpty();
        }
     ...
     * @return
     */
    private int getBottomEndY(boolean isLong) {
        if (isLong) {
    ...
    private int getTopStartY(boolean isLong) {
        if (isLong) {
            return dpFor05;
        } else {
            return mSmallLineHeight + dpFor05;
        }
    }

    private int getTopEndY() {
        return mLongLineHeight + dpFor05;
    }
